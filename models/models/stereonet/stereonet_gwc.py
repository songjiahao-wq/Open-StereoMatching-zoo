...
def make_gwc_volume(left, right, max_disp, num_groups=8):
    B, C, H, W = left.shape
    assert C % num_groups == 0
    channels_per_group = C // num_groups

    cost_volume = left.new_zeros(B, num_groups, max_disp, H, W)
    for d in range(max_disp):
        if d > 0:
            cost_volume[:, :, d, :, d:] = (
                left[:, :, :, d:].reshape(B, num_groups, channels_per_group, H, W - d)
                * right[:, :, :, :-d].reshape(B, num_groups, channels_per_group, H, W - d)
            ).mean(2)
        else:
            cost_volume[:, :, 0, :, :] = (
                left.reshape(B, num_groups, channels_per_group, H, W)
                * right.reshape(B, num_groups, channels_per_group, H, W)
            ).mean(2)
    return cost_volume
