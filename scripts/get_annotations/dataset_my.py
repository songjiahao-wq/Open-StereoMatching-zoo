...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='D:/BaiduNetdiskDownload/SceneFlow/', dstype='frames_finalpass',
                 things_test=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_datasets', default='only_sceneflow', choices=['sceneflow', 'kitti',
    ...
    with open('train_dataset.txt', 'w') as f:
        for img_pair, disp in zip(train_dataset.image_list, train_dataset.disparity_list):
            left_img = img_pair[0].replace('\\', '/').replace("D:/BaiduNetdiskDownload/SceneFlow/",'')
            right_img = img_pair[1].replace('\\', '/').replace("D:/BaiduNetdiskDownload/SceneFlow/",'')
            disp_path = disp.replace('\\', '/').replace("D:/BaiduNetdiskDownload/SceneFlow/",'')
